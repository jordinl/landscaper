...
const computeItems = (subcategories) => {
  return subcategories.map(subcategory => {
    ...
    const allItems = subcategory.allItems.map(item => ({ ...item, isVisible: filteredItems[item.id]  }))
    const itemsCount = allItems.reduce((count, item) => count + (item.large ? 4 : 1), 0)
    const largeItemsCount = allItems.reduce((count, item) => count + (item.large ? 1 : 0), 0)
...
export const calculateSize = categories => {
  const width = Math.max(...categories.map(({ style }) => style.left + style.width))
  const height = Math.max(...categories.map(({ style }) => style.top + style.height))
